...
const TransactionPage = () => {
  ...
  const loadMasterData = async () => {
    try {
      ...
      setMasterData({
        type: types.map(t => ({ value: t.name, label: t.description || t.name })),
        account: accounts.map(a => ({ value: a.name, label: `${a.name} (${a.type})` })),
        asset: assets.map(a => ({ value: a.symbol, label: `${a.symbol} - ${a.name}` })),
        tag: tags.map(t => ({ value: t.name, label: `${t.name} (${t.category})` })),
  ...
  const handleUpdateTransaction = async (transactionData) => {
    try {
      const updatedTransaction = await transactionApi.update(editingTransaction.id, transactionData)
  ...
  const handleInlineEdit = async (transactionId, field, newValue) => {
    try {
      ...
      console.log('Inline edit:', { transactionId, field, newValue, transaction })
      ...
      const updatedTransaction = await transactionApi.update(transactionId, updatedData)
        ...
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-500">
              Manage your financial transactions with dual-currency support
            </p>
          </div>
