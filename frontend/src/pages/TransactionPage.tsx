...
const TransactionPage: React.FC = () => {
    ...
    const [fxConversionCache, setFxConversionCache] = useState<
        Map<string, number>
    >(new Map());
    const [fxLoadingStates, setFxLoadingStates] = useState<
        Map<string, boolean>
    >(new Map());
                ...
                const mapped = (data ?? []).map((raw: unknown) => {
                    ...
                    const assetSym =
                        typeof assetObj === 'string'
                            ? assetObj
                            : ((assetObj as Record<string, unknown> | null)
                                  ?.symbol ?? '');
                    const type =
                    ...
                    const qty =
                        Number(rawRecord?.amount ?? rawRecord?.quantity ?? 0) ??
                        0;
                    // cashflow sign
                    let cashflow = 0;
        ...
        (
            ...
            if (!fxLoadingStates.get(rateKey)) {
                ...
                void fxService
                    ...
                    .then((rate) => {
                        setFxRateCache((prev) =>
                            new Map(prev).set(rateKey, rate)
                        );
                        const amt = amountLocal * rate;
                        setFxConversionCache((prev) =>
                            new Map(prev).set(cacheKey, amt)
                        );
                    })
                    .catch(() => {
                    ...
                    .finally(() => {
                        setFxLoadingStates((prev) =>
                            new Map(prev).set(rateKey, false)
                        );
                    });
            }
    ...
    const columns: TableColumn<Transaction>[] = [
        ...
        },
        {
            ...
            render: (value) => {
                ...
                try {
                    ...
                    if (v?.symbol && typeof v.symbol === 'string')
                        return v.symbol;
                } catch {
        ...
        },
        {
            ...
            render: (_value, _column, row) => {
                ...
                const type =
                    typeof _type === 'string' ? _type.toLowerCase() : '';
                const isNeutral = ['deposit', 'withdraw', 'borrow'].includes(
                    type
                );
                ...
                const convertedCashflow =
                    typeof _convertedCashflow === 'number'
                        ? _convertedCashflow
                        : 0;
                ...
                <div className="flex justify-between items-center">
                    <div>
                        ...
                        <p className="mt-1 text-sm text-gray-500">
                            Track your financial transactions with dual currency
                            valuation and comprehensive reporting.
                        </p>
                    </div>
