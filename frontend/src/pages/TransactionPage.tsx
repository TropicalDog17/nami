...
import { transactionApi, adminApi, actionsApi, pricesApi } from '../services/api';
...
const TransactionPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    (async ()=>{
      try {
        ...
        if (price && !cancelled) {
          setActionForm((s)=>({ ...s, params: { ...s.params, price_quote: String(price) }}));
        }
      } catch (e) {
  ...
  }, [actionForm.action, actionForm.params.base_asset, actionForm.params.quote_asset, actionForm.params.date, todayStr]);
  ...
  const columns: Column[] = [
    ...
    },
    {
      ...
      render: (value: any, _column: any, row?: any) => {
        ...
        const isInternal = Boolean(row?.internal_flow) && (type === 'transfer_in' || type === 'transfer_out');
        ...
        const isNeutral = isInternal || NEUTRAL_TYPES.includes(type) || (!isInflow && !isOutflow);
        ...
        return (
          <span
            ...
            title={isInternal ? 'Internal transfer' : isNeutral ? 'Neutral cash flow' : isInflow ? 'Inflow' : 'Outflow'}
          >
            {typeLabel}
          </span>
        );
    ...
    },
    {
      ...
      render: (_value: any, _column: any, row: any) => {
        ...
        if (isNeutral) {
          const amt = currency === 'USD' ? Number(row?.amount_usd || 0) : Number(row?.amount_vnd || 0);
          if (!amt) return '-';
          return <span className="font-medium text-gray-800">{numberFormatter.format(amt)}</span>;
        }
          ...
          {actionForm.action === 'spot_buy' ? (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
              ...
              <ComboBox
                ...
                onCreate={async (name)=>{ await adminApi.createAccount({ name, type:'exchange', is_active:true }); const accounts = await adminApi.listAccounts(); setMasterData((prev)=>({ ...prev, account: ((accounts as any[])||[]).map((a: any)=>({ value:a.name, label:`${a.name} (${a.type})`})) })); }}
              />
              <input className="px-3 py-2 border rounded" placeholder="Base Asset (e.g., BTC)" value={actionForm.params.base_asset || ''} onChange={(e)=>setActionForm(s=>({ ...s, params:{...s.params, base_asset:e.target.value}}))} />
              <input className="px-3 py-2 border rounded" placeholder="Quote Asset (e.g., USDT)" value={actionForm.params.quote_asset || ''} onChange={(e)=>setActionForm(s=>({ ...s, params:{...s.params, quote_asset:e.target.value}}))} />
              ...
              <input className="px-3 py-2 border rounded" placeholder="Price (quote)" value={actionForm.params.price_quote || ''} onChange={(e)=>setActionForm(s=>({ ...s, params:{...s.params, price_quote:e.target.value}}))} />
              {/* Fee mode */}
              <select className="px-3 py-2 border rounded" value={actionForm.params.fee_mode || ''} onChange={(e)=>setActionForm(s=>({ ...s, params:{...s.params, fee_mode:e.target.value}}))}>
              ...
              {actionForm.params.fee_mode === 'percent' && (
                <input className="px-3 py-2 border rounded" placeholder="Fee %" onChange={(e)=>setActionForm(s=>({ ...s, params:{...s.params, fee_percent:e.target.value}}))} />
              )}
              {actionForm.params.fee_mode === 'base' && (
                <input className="px-3 py-2 border rounded" placeholder="Fee (base)" onChange={(e)=>setActionForm(s=>({ ...s, params:{...s.params, fee_base:e.target.value}}))} />
              )}
              {actionForm.params.fee_mode === 'quote' && (
                <input className="px-3 py-2 border rounded" placeholder="Fee (quote)" onChange={(e)=>setActionForm(s=>({ ...s, params:{...s.params, fee_quote:e.target.value}}))} />
              )}
              <input className="px-3 py-2 border rounded" placeholder="Counterparty (optional)" onChange={(e)=>setActionForm(s=>({ ...s, params:{...s.params, counterparty:e.target.value}}))} />
            </div>
          ) : null}
