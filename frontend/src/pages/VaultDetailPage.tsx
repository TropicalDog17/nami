...
const VaultDetailPage: React.FC = () => {
    ...
    const [headerMetrics, setHeaderMetrics] =
        useState<VaultHeaderMetrics | null>(null);
    ...
    const [rewardCreateIncome, setRewardCreateIncome] =
        useState<boolean>(false);
        ...
        try {
            ...
            if (tokenizedData) {
                ...
                try {
                    const [h, tx] = await Promise.all([
                        vaultLedgerApi.holdings<Record<string, unknown>>(
                            vaultId
                        ),
                        vaultLedgerApi.transactions<
                    ...
                    const mapped = (Array.isArray(tx) ? tx : []).map(
                        (e: Record<string, unknown>) => {
                            ...
                            if (typeof assetRaw === 'string') {
                                ...
                                const symbolRaw = (
                                    assetRaw as Record<string, unknown>
                                ).symbol;
                                asset =
            ...
            if (!(apiErr instanceof ApiError && apiErr.status === 404)) {
                if (retryCount < MAX_RETRIES) {
                    ...
                    setError(message);
                    showErrorToast(
                        'Failed to load vault details after multiple retries'
                    );
                    setLoading(false);
                }
                return;
            ...
            try {
                ...
                const vaults = Array.isArray(vaultsData)
                    ? vaultsData
                          ...
                          .filter(
                              (v) => v.status === 'active' && v.id !== vaultId
                          )
                          .map((v) => ({
    ...
    if (isTokenizedVault && tokenizedVaultDetails) {
        ...
        const totalSupply = parseFloat(
            tokenizedVaultDetails.total_supply ?? '0'
        );
                ...
                <div className="mb-6 flex items-center justify-between">
                    <div>
                        ...
                        <h1 className="text-3xl font-bold text-gray-900 mb-1">
                            ...
                            {tokenizedVaultDetails.token_symbol})
                        </h1>
                        <p className="text-gray-600">
                            {tokenizedVaultDetails.description ??
                        ...
                        <CardContent>
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                ...
                                <div>
                                    ...
                                    <div className="text-xl font-semibold">
                                        {formatCurrency(
                                            headerMetrics?.deposits_cum_usd ?? 0
                                        )}
                                    </div>
                                    <div className="text-xs text-gray-500 mt-1">
                                        All deposits
                                ...
                                <div>
                                    ...
                                    <div className="text-xl font-semibold text-red-600">
                                        {formatCurrency(
                                            headerMetrics?.withdrawals_cum_usd ??
                                                0
                                        )}
                                    </div>
                                    <div className="text-xs text-gray-500 mt-1">
                            ...
                            <div className="mt-4 p-3 bg-blue-50 rounded-md">
                                <p className="text-sm text-blue-800">
                                    <strong>Simple Balance Tracking:</strong>{' '}
                                    Income - Expenses = Current Balance
                                </p>
                                <p className="text-xs text-blue-600 mt-1">
                                    No P&amp;L or ROI calculations for this
                                    account (it&apos;s cash, not an investment)
                                </p>
                            </div>
                        </CardContent>
                    </Card>
                                ...
                                <CardContent className="p-4">
                                    ...
                                    <p className="text-sm text-gray-600">
                                        {(() => {
                                            ...
                                            return typeof headerMetrics?.aum_usd ===
                                                'number' &&
                                                isFinite(headerMetrics.aum_usd)
                                                ? 'Backend rolling AUM'
                                                : typeof rollingAUM ===
                                ...
                                <Card>
                                    <CardContent className="p-4">
                                        ...
                                        <div style={{ height: 280 }}>
                                            <TimeSeriesLineChart
                                                labels={vaultSeries.map(
                                                    (p) => p.date
                                                )}
                                                datasets={[
                                                    {
                                                        label: 'PnL (USD)',
                                                        data: vaultSeries.map(
                                                            (p) => p.pnl_usd
                                                        ),
                                                        color: '#059669',
                                                        fill: true,
                    ...
                    <Card>
                        <CardContent className="p-4">
                            <ManualPricingControl
                                ...
                                onPricingModeChange={
                                    handleTokenizedPricingModeChange
                                }
                            />
                        </CardContent>
                    </Card>
                            ...
                            <dl className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
                                <div>
                                    ...
                                    <dd className="text-gray-900">
                                        {isValidDate(
                                            new Date(
                                                tokenizedVaultDetails.inception_date
                                            )
                                        )
                                            ? format(
                                                  new Date(
                                ...
                                <div>
                                    ...
                                    <dd className="text-gray-900">
                                        {isValidDate(
                                            new Date(
                                                tokenizedVaultDetails.last_updated
                                            )
                                        )
                                            ? format(
                                                  new Date(
                                    ...
                                    <ComboBox
                                        ...
                                        onChange={(value) => {
                                            if (showTokenizedDepositForm) {
                                                setTokenizedDepositAsset(
                                                    String(value)
                                                );
                                            } else {
                                                setTokenizedWithdrawAsset(
                                                    String(value)
                                                );
                                            }
                                        }}
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
                                ...
                                <div>
                                    ...
                                    <input
                                        ...
                                        onChange={(e) =>
                                            setRewardDestination(e.target.value)
                                        }
                                        className="w-full px-3 py-2 border border-gray-300 rounded-md"
                                        placeholder="Spend"
                                    />
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                                <label className="inline-flex items-center gap-2 text-sm text-gray-700">
                                    <input
                                        ...
                                        checked={rewardMark}
                                        onChange={(e) =>
                                            setRewardMark(e.target.checked)
                                        }
                                    />
                                ...
                                <dl className="grid grid-cols-2 gap-3 text-sm">
                                    <div>
                                        ...
                                        <dd className="text-gray-900">
                                            {(() => {
                                                const v =
                                                    ledgerHoldings.total_shares as unknown as
                                                        | string
                                                        | number
                                                        | undefined;
                                                const n =
                                    ...
                                    <div>
                                        ...
                                        <dd className="text-gray-900">
                                            {(() => {
                                                const v =
                                                    ledgerHoldings.total_aum as unknown as
                                                        | string
                                                        | number
                                                        | undefined;
                                                const n =
                                    ...
                                    <div>
                                        ...
                                        <dd className="text-gray-900">
                                            {(() => {
                                                const v =
                                                    ledgerHoldings.share_price as unknown as
                                                        | string
                                                        | number
                                                        | undefined;
                                                const n =
                                    ...
                                    <div>
                                        ...
                                        <dd className="text-gray-900">
                                            {ledgerHoldings.transaction_count ??
                                                0}
                                        </dd>
                                    </div>
