    ...
    useEffect(() => {
        ...
        const loadVaults = async () => {
            try {
                ...
                const list = await vaultApi.getActiveVaults<SimpleVault[]>({
                    is_open: true,
                });
                let arr = (list ?? [])
                    .map((v) => ({
                        name: (v.name ?? (v as { id?: string }).id) as string,
                        id: (v as { id?: string }).id ?? v.name,
                        status: v.status,
                    }))
                    .filter((v) => !!v.name);
                ...
                if (
                    defaultAccount &&
                    !arr.find((v) => v.name === defaultAccount)
                ) {
                    arr = [
                        {
                            name: defaultAccount,
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const transactionData = {
                ...
                amount_vnd: (
                    parseFloat(formData.amount || '0') * 24000
                ).toFixed(2),
                fee_usd: '0',
                fee_vnd: '0',
            };
                    ...
                    <div className="grid grid-cols-1 gap-3">
                        ...
                        <div className="space-y-2">
                            ...
                            <p className="text-xs text-muted-foreground">
                                Expenses are automatically withdrawn from your
                                Spend vault
                            </p>
                            {vaultsError ? (
                        ...
                        <div className="space-y-2">
                            ...
                            <Input
                                ...
                                onChange={(e) =>
                                    handleInputChange('payee', e.target.value)
                                }
