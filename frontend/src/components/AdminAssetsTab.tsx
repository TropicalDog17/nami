...
export const AdminAssetsTab: React.FC = () => {
    ...
    const fetchCurrentPrice = async (symbol: string, assetId: number) => {
        try {
            ...
            if (response.ok) {
                const data: { price: number }[] = (await response.json()) as {
                    price: number;
                }[];
    ...
    const handleCreate = async (data: unknown) => {
        try {
            ...
            const payload =
                data &&
                typeof data === 'object' &&
                'asset' in (data as Record<string, unknown>)
                    ? {
                          ...(data as AssetData).asset,
                          ...((data as AssetData).mapping
                              ? { mapping: (data as AssetData).mapping }
                              : {}),
                      }
                    : data;
            ...
            actions.setError(
                `Failed to create asset: ${(error as Error).message}`
            );
        }
    };
    ...
    const handleToggleActive = async (asset: Asset) => {
        try {
            ...
            actions.setSuccess(
                `Asset ${asset.is_active ? 'deactivated' : 'activated'}`
            );
            void loadAssets(); // added void
        } catch (error: unknown) {
            actions.setError(
                `Failed to update asset: ${(error as Error).message}`
            );
        }
    };
        ...
        <div className="space-y-4">
                                ...
                                >
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2">
                                        <button
                                            onClick={(e) => {
                                                e.stopPropagation();
                                                void handleToggleActive(asset);
                                            }} // added void and stopProp
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                ...
                <ul className="text-sm text-blue-800 space-y-1">
                    <li>
                        • Current prices are fetched automatically for{' '}
                        {`today's`} date
                    </li>
                    <li>
                        • Use {'Add Asset'} to configure custom price providers
                    </li>
                    <li>
                        • Enable auto-populate to fetch historical prices
                        automatically
                    </li>
