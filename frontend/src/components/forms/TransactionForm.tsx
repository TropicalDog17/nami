...
const TransactionForm = ({ transaction = null, onSubmit, onCancel }: Props) => {
        ...
        if (transaction) {
            setFormData({
                ...
                entry_date: transaction.entry_date
                    ? new Date(transaction.entry_date)
                          .toISOString()
                          .split('T')[0]
                    : '',
                exit_date: transaction.exit_date
                    ? new Date(transaction.exit_date)
                          .toISOString()
                          .split('T')[0]
                    : '',
    ...
    const normalizeNum = (s: string | null | undefined) =>
        String(s ?? '')
            .replace(/\s+/g, '')
            .replace(/,/g, '.');
    const effectiveQtyStr =
        quantityDraft !== '' ? quantityDraft : String(formData.quantity ?? '');
    const effectivePriceStr =
        priceLocalDraft !== ''
            ? priceLocalDraft
            : String(formData.price_local ?? '');
    ...
    const validateForm = () => {
        ...
        if (
            effectiveQtyStr &&
            isNaN(parseFloat(normalizeNum(effectiveQtyStr)))
        ) {
            newErrors.quantity = 'Quantity must be a number';
        }
        if (
            effectivePriceStr &&
            isNaN(parseFloat(normalizeNum(effectivePriceStr)))
        ) {
        ...
        if (
            effectivePriceStr &&
            parseFloat(normalizeNum(effectivePriceStr)) < 0
        ) {
    ...
    const handleSubmit = (e: FormEvent<HTMLFormElement>) => {
        ...
        const transactionData = {
            ...
            entry_date: formData.entry_date
                ? toISODateTime(formData.entry_date)
                : null,
            exit_date: formData.exit_date
                ? toISODateTime(formData.exit_date)
                : null,
    ...
    const InputField: FC<InputFieldProps> = ({
        ...
        <div className="space-y-2">
            <Label
                htmlFor={field}
                className={errors[field] ? 'text-destructive' : ''}
            >
                {label}
            ...
            {options ? (
                <Select
                    ...
                    onValueChange={(value) =>
                        handleInputChange(field as keyof FormDataType, value)
                    }
                >
                    <SelectTrigger
                        id={field}
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                <div className="border-b border-border pb-4">
                    ...
                    <p className="mt-1 text-sm text-muted-foreground">
                        Fill in the transaction details. Required fields are
                        marked with *.
                    </p>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    ...
                    <InputField
                        ...
                        options={assets
                            .filter(
                                (a: unknown) =>
                                    (a as { is_active: boolean }).is_active
                            )
                            .map((a: unknown) => {
                                const typedA = a as {
                                    symbol: string;
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    <div className="space-y-2">
                        <Label
                            htmlFor="account"
                            className={errors.account ? 'text-destructive' : ''}
                        >
                            Account
                ...
                <div className="border-t border-border pt-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        ...
                        <div className="space-y-2">
                            <Label
                                className={
                                    errors.price_local ? 'text-destructive' : ''
                                }
                            >
                            ...
                            <Input
                                ...
                                onChange={(e: ChangeEvent<HTMLInputElement>) =>
                                    setPriceLocalDraft(e.target.value)
                                }
                                onBlur={() => {
                                    if (priceLocalDraft !== '') {
                                        handleInputChange(
                                            'price_local',
                                            normalizeNum(priceLocalDraft)
                                        );
                                        setPriceLocalDraft('');
                                    }
                                }}
                        ...
                        <div className="space-y-2">
                            ...
                            <Input
                                ...
                                readOnly
                                value={
                                    Number.isFinite(amountLocalNum)
                                        ? amountLocalNum.toFixed(8)
                                        : ''
                                }
                ...
                <div className="border-t border-border pt-4">
                    ...
                    <div className="flex items-center space-x-2">
                        ...
                        />
                        <Label
                            htmlFor="internal_flow"
                            className="cursor-pointer"
                        >
                            Treat as internal P2P trade (zero cash flow for
                            Buy/Sell)
                        </Label>
                    </div>
                </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                        ...
                        <div className="space-y-2">
                            ...
                            <Input
                                ...
                                readOnly
                                value={
                                    Number.isFinite(amountUsdNum)
                                        ? amountUsdNum.toFixed(2)
                                        : ''
                                }
                                placeholder="Auto-calculated"
                                className="bg-muted"
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        ...
                        <div className="space-y-2">
                            ...
                            <DateInput
                                ...
                                onChange={(v: string) =>
                                    handleInputChange('entry_date', v)
                                }
                            />
                        </div>
