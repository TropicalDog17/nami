...
const DateInput: React.FC<DateInputProps> = ({
  ...
  const [inputValue, setInputValue] = useState<string>(toDateOnlyString(value));
  const [viewMonth, setViewMonth] = useState<number>(() => (inputValue ? fromYmd(inputValue).getMonth() : new Date().getMonth()));
  const [viewYear, setViewYear] = useState<number>(() => (inputValue ? fromYmd(inputValue).getFullYear() : new Date().getFullYear()));
    ...
    return (
      <div className="absolute z-20 mt-1 w-72 rounded-md border bg-white shadow-lg p-3">
        <div className="flex items-center justify-between mb-2">
          <button type="button" className="px-2 py-1 rounded hover:bg-gray-100" onClick={() => navigateMonth(-1)} aria-label="Previous month">‹</button>
          <div className="text-sm font-medium">{new Date(viewYear, viewMonth, 1).toLocaleString(undefined, { month: 'long', year: 'numeric' })}</div>
          <button type="button" className="px-2 py-1 rounded hover:bg-gray-100" onClick={() => navigateMonth(1)} aria-label="Next month">›</button>
        </div>
        ...
        <div className="flex items-center justify-between">
          <div className="space-x-2">
            <button type="button" className="px-2 py-1 text-xs border rounded hover:bg-gray-50" onClick={() => selectDate(todayYmd)} disabled={!isWithinBounds(todayYmd, min, max)}>Today</button>
            <button type="button" className="px-2 py-1 text-xs border rounded hover:bg-gray-50" onClick={() => { const ymd = toDateOnlyString(new Date()); setViewMonth(new Date().getMonth()); setViewYear(new Date().getFullYear()); setInputValue(ymd); onChange?.(ymd); }}>Set</button>
          </div>
          <button type="button" className="px-2 py-1 text-xs text-gray-600 hover:text-gray-800" onClick={() => setOpen(false)}>Close</button>
        </div>
      </div>
    );
  };
