...
const ManualPricingControl: React.FC<ManualPricingControlProps> = ({
    ...
    const safeTotalValue = Number.isFinite(currentTotalValue)
        ? currentTotalValue
        : 0;
        ...
        try {
            ...
            const message =
                err instanceof Error
                    ? err.message
                    : 'Failed to enable manual pricing';
    ...
    const handleDisableManualPricing = async () => {
        if (
            !confirm(
                'Are you sure you want to disable manual pricing? The price will be determined by market data.'
            )
        ) {
            return;
        }
        ...
        try {
            ...
            const message =
                err instanceof Error
                    ? err.message
                    : 'Failed to disable manual pricing';
            showErrorToast(message);
        } finally {
            setLoading(false);
        }
    ...
    const handleUpdateTotalValue = async (e: React.FormEvent) => {
        ...
        const netContributionDelta = netContribution
            ? parseFloat(netContribution)
            : 0;
        if (isNaN(totalValue) || totalValue <= 0) {
            showErrorToast(
                'Please enter a valid total vault value greater than 0'
            );
            return;
        }
        if (isNaN(netContributionDelta)) {
            showErrorToast(
                'Please enter a valid net deposit/withdrawal amount'
            );
            return;
        }
        ...
        try {
            const response =
                await tokenizedVaultApi.updateTotalValue<VaultMetricsResponse>(
                    vaultId,
                    {
                        total_value: totalValue,
                        net_contribution_delta:
                            netContributionDelta !== 0
                                ? netContributionDelta
                                : undefined,
            ...
            const updatedPrice =
                response !== null && response !== undefined
                    ? parseFloat(
                          response.current_share_price ??
                              response.manual_price_per_share ??
                              `${currentPrice}`
                      )
                    : Number.NaN;
            const updatedValue =
                response !== null && response !== undefined
                    ? parseFloat(
                          response.total_assets_under_management ??
                              `${totalValue}`
                      )
                    : Number.NaN;
            ...
            const message =
                err instanceof Error
                    ? err.message
                    : 'Failed to update total vault value';
            showErrorToast(message);
        } finally {
            setLoading(false);
        }
    ...
    return (
        <div className="bg-gray-50 p-4 rounded-lg">
            <div className="flex items-center justify-between mb-4">
                <div>
                    ...
                    <p className="text-xs text-gray-500">
                        ...
                        {isManualPricing && '(Manual)'}
                    </p>
                    <p className="text-xs text-gray-500">
                        Total vault value: $
                        {safeTotalValue.toLocaleString(undefined, {
            ...
            {showUpdateForm && (
                <form
                    onSubmit={(e) => void handleUpdateTotalValue(e)}
                    className="space-y-3"
                >
                    <div>
                        ...
                        {safeTotalSupply > 0 ? (
                            <p className="text-xs text-gray-500 mt-1">
                                Based on{' '}
                                {safeTotalSupply.toLocaleString(undefined, {
                                    maximumFractionDigits: 2,
                                })}{' '}
                                tokens outstanding, this sets the price to $
                                {newTotalValue
                                    ? (
                                          parseFloat(newTotalValue) /
                                          safeTotalSupply
                                      ).toFixed(4)
                                    : (
                                          safeTotalValue / safeTotalSupply
                                      ).toFixed(4)}
                                .
                            </p>
                        ) : (
                            <p className="text-xs text-yellow-700 mt-1">
                                Token price will update once shares are minted.
                                For now the vault value snapshot is stored.
                            </p>
                        )}
                    </div>
                    ...
                    <div>
                        ...
                        />
                        <p className="text-xs text-gray-500 mt-1">
                            Positive numbers increase capital (deposits),
                            negative numbers decrease capital (withdrawals). The
                            share price is adjusted using net performance only.
                        </p>
                    </div>
            ...
            {isManualPricing && !showUpdateForm && (
                <div className="text-xs text-gray-600">
                    <p className="font-medium text-purple-700">
                        Manual Pricing Active
                    </p>
                    <p>
                        Provide total value plus net deposits/withdrawals so
                        price reflects pure performance; every update is logged.
                    </p>
                </div>
            )}
        </div>
