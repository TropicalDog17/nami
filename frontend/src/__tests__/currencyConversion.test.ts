...
describe('Currency Conversion Logic', () => {
    it('should convert amounts using historical FX rates', () => {
        ...
        expect(convertAmount(transaction1, 'USD')).not.toBe(
            convertAmount(transaction2, 'USD')
        );
        expect(convertAmount(transaction1, 'VND')).not.toBe(
            convertAmount(transaction2, 'VND')
        );
    });
    ...
    it('should format currency amounts correctly', () => {
        ...
        expect(formatCurrency(1100.5, 'USD')).toBe('$1,100.50');
    });
});
