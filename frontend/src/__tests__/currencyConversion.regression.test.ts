...
describe('Currency Conversion Regression Tests', () => {
  describe('GitHub Issue #2: Currency switching doesn\'t apply historical FX conversion', () => {
    ...
    it('should handle malformed data gracefully', () => {
      ...
      expect(convertWithFallback(malformedTransactions[0], 'USD')).toBe(1000);
      expect(convertWithFallback(malformedTransactions[0], 'VND')).toBe(24000000);
      expect(convertWithFallback(malformedTransactions[1], 'USD')).toBe(0);
      expect(convertWithFallback(malformedTransactions[1], 'VND')).toBe(0);
      expect(convertWithFallback(malformedTransactions[2], 'USD')).toBe(0);
      expect(convertWithFallback(malformedTransactions[2], 'VND')).toBe(0);
    });
  });
