...
type Action =
    ...
    | {
          ...
          payload: TransactionType & { id?: string | number };
      }
        ...
        case ActionTypes.UPDATE_TRANSACTION_TYPE:
            return {
                ...
                transactionTypes: state.transactionTypes.map((type) =>
                    type.id ===
                    (
                        action.payload as TransactionType & {
                            id: string | number;
                        }
                    ).id
                        ? action.payload
        ...
        case ActionTypes.UPDATE_ACCOUNT:
            return {
                ...
                accounts: state.accounts.map((account) =>
                    account.id ===
                    (action.payload as Account & { id: string | number }).id
                        ? action.payload
                        : account
                ),
            };
        ...
        case ActionTypes.UPDATE_ASSET:
            return {
                ...
                assets: state.assets.map((asset) =>
                    asset.id ===
                    (action.payload as Asset & { id: string | number }).id
                        ? action.payload
                        : asset
                ),
            };
        ...
        case ActionTypes.UPDATE_TAG:
            return {
                ...
                tags: state.tags.map((tag) =>
                    tag.id ===
                    (action.payload as Tag & { id: string | number }).id
                        ? action.payload
                        : tag
                ),
            };
        ...
        () => ({
            ...
            loadTransactionTypes: async () => {
                try {
                    ...
                    const types =
                        (await adminApi.listTypes()) as TransactionType[];
                    dispatch({
                    ...
                    const msg =
                        (err as Error)?.message ?? 'Failed to load types';
                    dispatch({ type: ActionTypes.SET_ERROR, payload: msg });
                } finally {
            ...
            loadAccounts: async () => {
                try {
                    dispatch({ type: ActionTypes.SET_LOADING, payload: true });
                    const accountsData =
                        (await adminApi.listAccounts()) as Account[];
                    dispatch({
                    ...
                    const msg =
                        (err as Error)?.message ?? 'Failed to load accounts';
                    dispatch({ type: ActionTypes.SET_ERROR, payload: msg });
                } finally {
                    dispatch({ type: ActionTypes.SET_LOADING, payload: false });
                }
            ...
            loadAssets: async () => {
                try {
                    ...
                    const msg =
                        (err as Error)?.message ?? 'Failed to load assets';
                    dispatch({ type: ActionTypes.SET_ERROR, payload: msg });
                } finally {
                    dispatch({ type: ActionTypes.SET_LOADING, payload: false });
                }
            ...
            loadTags: async () => {
                try {
                    ...
                    const msg =
                        (err as Error)?.message ?? 'Failed to load tags';
                    dispatch({ type: ActionTypes.SET_ERROR, payload: msg });
                } finally {
                    dispatch({ type: ActionTypes.SET_LOADING, payload: false });
                }
            ...
            loadAllMasterData: async () => {
                try {
                    ...
                    const msg =
                        (err as Error)?.message ?? 'Failed to load data';
                    dispatch({ type: ActionTypes.SET_ERROR, payload: msg });
                } finally {
                    dispatch({ type: ActionTypes.SET_LOADING, payload: false });
                }
            ...
            createTransactionType: async (type: TransactionType) => {
                try {
                    const newType = (await adminApi.createType(
                        type
                    )) as TransactionType;
                    dispatch({
                        type: ActionTypes.ADD_TRANSACTION_TYPE,
                        payload: newType,
                    });
                    return newType;
                } catch (err: unknown) {
                    const msg =
                        (err as Error)?.message ?? 'Failed to create type';
                    dispatch({ type: ActionTypes.SET_ERROR, payload: msg });
                    throw err;
                }
            },
            ...
            updateTransactionType: async (
                ...
                try {
                    const updatedType = (await adminApi.updateType(
                        id,
                        type
                    )) as TransactionType & { id: string | number };
                    dispatch({
                    ...
                    const msg =
                        (err as Error)?.message ?? 'Failed to update type';
                    dispatch({ type: ActionTypes.SET_ERROR, payload: msg });
                    throw err;
                }
            },
            ...
            deleteTransactionType: async (id: string | number) => {
                try {
                    ...
                    dispatch({
                        type: ActionTypes.DELETE_TRANSACTION_TYPE,
                        payload: id,
                    });
                } catch (err: unknown) {
                    const msg =
                        (err as Error)?.message ?? 'Failed to delete type';
                    dispatch({ type: ActionTypes.SET_ERROR, payload: msg });
                    throw err;
                }
            },
            ...
            updateAccount: async (id: string | number, account: Account) => {
                try {
                    const updatedAccount = (await adminApi.updateAccount(
                        id,
                        account
                    )) as Account & { id: string | number };
                    dispatch({
                    ...
                    const msg =
                        (err as Error)?.message ?? 'Failed to update account';
                    dispatch({ type: ActionTypes.SET_ERROR, payload: msg });
                    throw err;
                }
            },
            ...
            createAsset: async (asset: Asset) => {
                try {
                    const newAsset = (await adminApi.createAsset(
                        asset
                    )) as Asset;
                    dispatch({
                    ...
                    const msg =
                        (err as Error)?.message ?? 'Failed to create asset';
                    dispatch({ type: ActionTypes.SET_ERROR, payload: msg });
                    throw err;
                }
            },
            ...
            updateAsset: async (id: string | number, asset: Asset) => {
                try {
                    const updatedAsset = (await adminApi.updateAsset(
                        id,
                        asset
                    )) as Asset & { id: string | number };
                    dispatch({
                    ...
                    const msg =
                        (err as Error)?.message ?? 'Failed to update asset';
                    dispatch({ type: ActionTypes.SET_ERROR, payload: msg });
                    throw err;
                }
            },
            ...
            deleteAsset: async (id: string | number) => {
                try {
                    ...
                    const msg =
                        (err as Error)?.message ?? 'Failed to delete asset';
                    dispatch({ type: ActionTypes.SET_ERROR, payload: msg });
                    throw err;
                }
            },
            ...
            createTag: async (tag: Tag) => {
                try {
                    ...
                    const msg =
                        (err as Error)?.message ?? 'Failed to create tag';
                    dispatch({ type: ActionTypes.SET_ERROR, payload: msg });
                    throw err;
                }
            },
            ...
            updateTag: async (id: string | number, tag: Tag) => {
                try {
                    const updatedTag = (await adminApi.updateTag(
                        id,
                        tag
                    )) as Tag & { id: string | number };
                    dispatch({
                    ...
                    const msg =
                        (err as Error)?.message ?? 'Failed to update tag';
                    dispatch({ type: ActionTypes.SET_ERROR, payload: msg });
                    throw err;
                }
            },
