...
class FXRateService {
    ...
    async getFXRate(from: string, to: string, date?: Date): Promise<number> {
        ...
        const fetchPromise = (async (): Promise<number> => {
            try {
                let rate: number;
                if (dateStr) {
                    const response = (await fxApi.getHistoricalRate(
                        fromUpper,
                        toUpper,
                        dateStr
                    )) as Array<{ rate?: number; Rate?: number }>;
                    if (
                        response &&
                    ...
                    const response = (await fxApi.getTodayRate(
                        fromUpper,
                        toUpper
                    )) as { rate?: number; Rate?: number };
                ...
                if (fromUpper === 'USD' && toUpper === 'VND')
                    fallback = this.FALLBACK_VND_RATE;
                else if (fromUpper === 'VND' && toUpper === 'USD')
                    fallback = 1 / this.FALLBACK_VND_RATE;
                else if (toUpper === 'VND')
                    fallback = this.FALLBACK_VND_RATE; // generic best-effort
                else fallback = 1.0;
    ...
    async convertAmount(
        ...
        try {
            ...
            console.error(
                `Currency conversion failed ${amount} ${from} to ${to}:`,
                error
            );
    ...
    async batchConvert(
        ...
        for (const item of amounts) {
            ...
            if (cachedRate === null) {
                ...
                if (!ratePromises.has(requestKey)) {
                    ratePromises.set(
                        requestKey,
                        this.getFXRate(from, to, item.date)
                    );
                }
            }
        }
        ...
        const rateResults = await Promise.allSettled(
            Array.from(ratePromises.values())
        );
