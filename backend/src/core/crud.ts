...
export function createCrudRouter<T extends Entity, CreateInput, UpdateInput>(
  ...
  const router = Router();
  const { basePath, resourceName, operations, validateCreate, validateUpdate } =
    config;
 ...
 */
export interface NumberIdCrudConfig<
  T extends Entity,
  CreateInput,
  UpdateInput,
 ...
 */
export function createNumberIdCrudRouter<
  T extends Entity,
  CreateInput,
  UpdateInput,
>(config: NumberIdCrudConfig<T, CreateInput, UpdateInput>): Router {
  ...
  const adaptedOperations: CrudOperations<T, CreateInput, UpdateInput> = {
    ...
    update: (id: number | string, input: UpdateInput) =>
      operations.update(id as number, input),
