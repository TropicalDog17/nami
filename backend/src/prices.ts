...
// Helpers
const CRYPTO_SET = new Set(['BTC','ETH','SOL','USDT','USDC','BNB','XRP','ADA','DOGE','TRX','DOT','MATIC','AVAX']);
...
function dateOnly(d: Date) { return d.toISOString().split('T')[0]; }
...
pricesRouter.get('/fx/history', async (req, res) => {
  try {
    ...
    const days: string[] = [];
    const iter = new Date(Date.UTC(start.getUTCFullYear(), start.getUTCMonth(), start.getUTCDate()));
    ...
    const results = [] as Array<{ date: string; rate: number; from: string; to: string }>;
