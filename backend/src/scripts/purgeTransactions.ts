...
function run() {
  ...
  // Purge transactions
  const before = Array.isArray(data.transactions) ? data.transactions.length : 0;
  data.transactions = [];
  ...
  console.log(`Purged ${before} transactions. Kept ${data.vaultEntries.length} vault entries.`);
  console.log('Done.');
}
