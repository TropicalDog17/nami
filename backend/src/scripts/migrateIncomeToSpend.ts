...
async function migrateIncomeToSpend() {
  ...
  const backupPath = path.join(
    __dirname,
    "../../data",
    `store.json.bak-income-migration-${timestamp}`,
  );

  // Step 1: Backup
  console.log("Step 1: Creating backup...");
  fs.copyFileSync(path.join(__dirname, "../../data/store.json"), backupPath);
  ...
  if (incomeEntries.length > 0) {
    ...
    incomeEntries.forEach((entry) => {
      console.log(
        `  - ${entry.type} | ${entry.amount} ${entry.asset.symbol} | ${new Date(entry.at).toLocaleDateString()} | ${entry.note || "No note"}`,
      );
    });
    console.log("");
  }
  ...
  console.log(
    `Migrated ${incomeEntries.length} vault entries from Income to Spend`,
  );
