...
export class SettingsRepositoryJson implements ISettingsRepository {
  ...
  getBorrowingSettings(): BorrowingSettings {
    ...
    const rate = typeof settings?.borrowingMonthlyRate === 'number'
      ? settings.borrowingMonthlyRate
      : 0.02;
    const lastAccrualStart = settings?.borrowingLastAccrualAt || new Date().toISOString();
  ...
  updateBorrowingSettings(settings: Partial<BorrowingSettings>): void {
    ...
    if (settings.rate !== undefined) this.setSetting('borrowingMonthlyRate', String(settings.rate));
    if (settings.lastAccrualStart) this.setSetting('borrowingLastAccrualAt', settings.lastAccrualStart);
  }
...
export class SettingsRepositoryDb extends BaseDbRepository implements ISettingsRepository {
  getSettings(): StoreShape['settings'] {
  ...
  getSetting(key: string): string | undefined {
    const row = this.findOne('SELECT value FROM settings WHERE key = ?', [key], (r: any) => r.value);
    return row;
  }
  ...
  getBorrowingSettings(): BorrowingSettings {
    ...
    const lastAccrualStart = this.getSetting('borrowingLastAccrualAt') || new Date().toISOString();
