...
aiRouter.post(
  ...
  async (req: Request, res: Response) => {
    try {
      const { vnd_amount, date, counterparty, tag, note, source_ref } =
        req.body || {};

      if (!vnd_amount || typeof vnd_amount !== "number" || vnd_amount <= 0) {
        return res
          .status(400)
          .json({ error: "vnd_amount is required and must be positive" });
      }
...
aiRouter.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const account =
        credit_account || settingsRepository.getDefaultSpendingVaultName();
      vaultService.ensureVault(account);
      ...
      res
        .status(400)
        .json({ error: e?.message || "Failed to record credit expense" });
    }
  },
);
...
aiRouter.post(
  ...
  async (req: Request, res: Response) => {
    try {
      const { vnd_amount, date, from_account, to_credit_account, note } =
        req.body || {};
      ...
      const account =
        from_account || settingsRepository.getDefaultSpendingVaultName();
      vaultService.ensureVault(account);
      ...
      res
        .status(400)
        .json({ error: e?.message || "Failed to record card payment" });
    }
  },
);
