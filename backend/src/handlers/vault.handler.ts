...
vaultsRouter.get("/vaults", async (req: Request, res: Response) => {
    ...
    if (tokenized) {
        try {
            ...
            return res.status(500).json({ error: e?.message || "Failed to list vaults" });
        }
    }
    ...
    const enriched = await Promise.all(
        list.map(async (v) => {
            ...
            return {
                ...
                name: v.name,
                status:
                    v.status.toLowerCase() === "active" ? "active" : "closed",
                inception_date: v.createdAt,
...
vaultsRouter.post(
    ...
    async (req: Request, res: Response) => {
        try {
            ...
            const quantity =
                Number(req.body?.quantity ?? req.body?.amount ?? 0) || 0;
            const value =
                Number(
    ...
    async (req: Request, res: Response) => {
        try {
            ...
            if (shouldMark) {
                const stats = await vaultService.vaultStats(name);
                const intended =
                    Number(req.body?.new_total_usd ?? 0) ||
                    stats.aumUSDManual + amount;
                const asset = { type: "FIAT", symbol: "USD" } as const;
                vaultService.addVaultEntry({
                    vault: name,
                    type: "VALUATION",
            ...
            const createIncome =
                String(req.body?.create_income || "").toLowerCase() ===
                    "true" || req.body?.create_income === true;
            if (createIncome) {
