...
adminRouter.get("/admin/pending-actions", (req: Request, res: Response) => {
  ...
  const result = status
    ? adminRepository.findPendingActionsByStatus(status)
    : adminRepository.findAllPendingActions();
  res.json(result);
});
...
adminRouter.delete("/admin/pending-actions", (req: Request, res: Response) => {
  ...
  const count = adminRepository.deletePendingActionsByStatus(status, batchId);
