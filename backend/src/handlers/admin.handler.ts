...
adminRouter.post('/admin/settings/income-vault', (req: Request, res: Response) => {
  try {
    const name = String(req.body?.name || '').trim();
    if (!name) return res.status(400).json({ error: 'name is required' });
    ...
    res.status(400).json({ error: e?.message || 'failed to set income vault' });
  }
});
...
adminRouter.get('/admin/pending-actions', (req: Request, res: Response) => {
  ...
  res.json(adminRepository.findAllPendingActions(status));
});

// Bulk operations MUST come before :id routes (Express route order matters)
adminRouter.post('/admin/pending-actions/reject-all', (req: Request, res: Response) => {
  const batchId = req.query.batch_id as string | undefined;
  const count = adminRepository.rejectAllPending(batchId);
  res.json({ ok: true, rejected: count });
});

adminRouter.post('/admin/pending-actions/accept-all', async (req: Request, res: Response) => {
  const batchId = req.query.batch_id as string | undefined;

  // Get all pending actions to process
  const allPending = adminRepository.findAllPendingActions('pending');
  const toProcess = batchId
    ? allPending.filter(p => p.batch_id === batchId)
    : allPending;
  ...
  for (const pending of toProcess) {
    try {
      ...
      if (actionJson && typeof actionJson === 'object') {
        const { action, params } = actionJson as { action?: string; params?: any };
        ...
        if (action && params) {
          ...
          switch (action) {
            case 'spend_vnd':
              vaultService.ensureVault(params.account || settingsRepository.getDefaultSpendingVaultName());
              ...
              adminRepository.updatePendingAction(pending.id, {
                status: 'accepted',
            ...
            case 'income_vnd':
              vaultService.ensureVault(params.account || settingsRepository.getDefaultIncomeVaultName());
            ...
            case 'credit_spend_vnd':
              ...
              tx = await transactionService.createExpenseTransaction({
                ...
                at: params.date,
                account: params.account,
                note: params.note ? `[Credit] ${params.note}` : '[Credit Card Expense]',
                category: params.tag,
                counterparty: params.counterparty,
              });
              adminRepository.updatePendingAction(pending.id, {
            ...
            case 'card_payment_vnd':
              const spendingAccount = params.from_account || settingsRepository.getDefaultSpendingVaultName();
...
adminRouter.delete('/admin/pending-actions', (req: Request, res: Response) => {
  ...
  if (!status) {
    return res.status(400).json({ error: 'status query param required (pending|accepted|rejected)' });
  }

  const count = adminRepository.deleteAllByStatus(status, batchId);
...
adminRouter.get('/admin/pending-actions/:id', (req: Request, res: Response) => {
  ...
  const item = adminRepository.findPendingActionById(id);
...
adminRouter.post('/admin/pending-actions', (req: Request, res: Response) => {
  try {
    const { source, raw_input, toon_text, action_json, confidence, batch_id, meta } = req.body || {};
    ...
    const created = adminRepository.createPendingAction({
    ...
    res.status(400).json({ error: e?.message || 'Failed to create pending action' });
  }
});

adminRouter.post('/admin/pending-actions/:id/accept', async (req: Request, res: Response) => {
  const id = String(req.params.id);

  // Get the pending action
  const pending = adminRepository.findPendingActionById(id);
  if (!pending) return res.status(404).json({ error: 'Pending action not found' });
  if (pending.status !== 'pending') {
    return res.status(400).json({ error: 'Only pending actions can be accepted' });
  }
  ...
  const updated = adminRepository.updatePendingAction(id, {
  ...
  if (!updated) return res.status(404).json({ error: 'Pending action not found' });
  res.json({ ok: true, item: updated });
});

adminRouter.post('/admin/pending-actions/:id/reject', (req: Request, res: Response) => {
  const id = String(req.params.id);
  const updated = adminRepository.updatePendingAction(id, { status: 'rejected' });
  if (!updated) return res.status(404).json({ error: 'Pending action not found' });
  res.json({ ok: true, item: updated });
});

adminRouter.delete('/admin/pending-actions/:id', (req: Request, res: Response) => {
  const id = String(req.params.id);
  const ok = adminRepository.deletePendingAction(id);
