...
import { adminRepository, pendingActionsRepository, settingsRepository } from '../repositories';
...
adminRouter.post('/admin/pending-actions/reject-all', (req: Request, res: Response) => {
  ...
  const toReject = batchId
    ? pendingActions.filter((p: { batch_id?: string }) => p.batch_id === batchId)
    : pendingActions;
  let count = 0;
  for (const action of toReject) {
    const updated = pendingActionsRepository.update(action.id, { status: 'rejected' });
    if (updated) count++;
  }
