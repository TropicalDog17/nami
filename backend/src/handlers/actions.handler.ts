        ...
        switch (action) {
            case "spot_buy": {
                ...
                if (!base || !quote || !(quantity > 0)) {
                    return res
                        .status(400)
                        .json({ error: "Invalid spot_buy params" });
                }
                const feePercent = params?.fee_percent
                    ? Number(params.fee_percent)
                    : 0;
                let priceQuote = params?.price_quote
                    ? Number(params.price_quote)
                    : NaN;
                const atISO = toISODate(params?.date);

                const baseAsset: Asset = {
                    type:
                        base === "USD" || base.length === 3 ? "FIAT" : "CRYPTO",
                    symbol: base,
                };
                const quoteAsset: Asset = {
                    type:
                        quote === "USD" || quote.length === 3
                            ? "FIAT"
                            : "CRYPTO",
                    symbol: quote,
                };
                ...
                if (!isFinite(priceQuote) || priceQuote <= 0) {
                    ...
                    const baseRate = await priceService.getRateUSD(
                        baseAsset,
                        atISO
                    );
                    const quoteRate = await priceService.getRateUSD(
                        quoteAsset,
                        atISO
                    );
                ...
                const totalQuoteSpent =
                    grossQuoteSpent *
                    (1 + (isFinite(feePercent) ? feePercent / 100 : 0));
                ...
                const expenseTx: Transaction = {
                    id: uuidv4(),
                    type: "EXPENSE",
                    note: `spot_buy cost ${totalQuoteSpent} ${quote}${isFinite(feePercent) && feePercent > 0
                            ? ` (+${feePercent}% fee)`
                            : ""
                        }`,
                    asset: quoteAsset,
            ...
            case "init_balance": {
                ...
                const atISO = toISODate(params?.date);
                const account: string | undefined = params?.account ? String(params.account) : undefined;
                const note: string | undefined = params?.note ? String(params.note) : undefined;
                ...
                const rate = rateUSD && isFinite(rateUSD) && rateUSD > 0
                    ? { asset, rateUSD, timestamp: (atISO ?? new Date().toISOString()), source: "FIXED" as const }
            ...
            case "transfer": {
                ...
                const asset: Asset = {
                    type: assetSymbol === "USD" || assetSymbol.length === 3 ? "FIAT" : "CRYPTO",
                    symbol: assetSymbol,
                };

                // Destination asset (for cross-currency)
                const toAssetSymbol = params?.to_asset ? String(params.to_asset).toUpperCase() : assetSymbol;
                const toAsset: Asset = {
                    type: toAssetSymbol === "USD" || toAssetSymbol.length === 3 ? "FIAT" : "CRYPTO",
                    symbol: toAssetSymbol,
                };

                const toQuantity = params?.to_amount ? Number(params.to_amount) : quantity; // Default to 1:1 if not specified (will be fixed by rate if same asset, but if different need input or rate)
                ...
                const txOut: Transaction = {
                    ...
                    createdAt: atISO ?? new Date().toISOString(),
                    account: fromAccount,
                    note: note ? `Transfer to ${toAccount}: ${note}` : `Transfer to ${toAccount}`,
                    transferId,
                    rate: rateFrom,
                ...
                const txIn: Transaction = {
                    ...
                    createdAt: atISO ?? new Date().toISOString(),
                    account: toAccount,
                    note: note ? `Transfer from ${fromAccount}: ${note}` : `Transfer from ${fromAccount}`,
                    transferId,
                    rate: rateTo,
