...
transactionsRouter.post("/transactions/initial", async (req: Request, res: Response) => {
  try {
    const body: InitialRequest = InitialRequestSchema.parse(req.body);
    const results = await transactionService.createInitialTransactions(body.items);
...
transactionsRouter.post("/transactions/income", async (req: Request, res: Response) => {
  try {
...
transactionsRouter.post("/transactions/expense", async (req: Request, res: Response) => {
  try {
    const body: IncomeExpenseRequest = IncomeExpenseSchema.parse(req.body);
...
transactionsRouter.post("/transactions/borrow", async (req: Request, res: Response) => {
  try {
...
transactionsRouter.post("/transactions/loan", async (req: Request, res: Response) => {
  try {
    const body: BorrowLoanRequest = BorrowLoanSchema.parse(req.body);
...
transactionsRouter.post("/transactions/repay", async (req: Request, res: Response) => {
  try {
...
// Delete transaction
transactionsRouter.delete("/transactions/:id", (req: Request, res: Response) => {
...
transactionsRouter.post("/transactions", async (req: Request, res: Response) => {
  try {
    ...
    if (type === "deposit" || type === "withdraw") {
      ...
      const at = String(body.date || body.at || '') || undefined;
      const account = body.account ? String(body.account) : vaultService.getDefaultSpendingVaultName();
    ...
    if (type === "income" || type === "expense") {
      ...
      const tx = type.toUpperCase() === 'INCOME'
    ...
    if (type === "sell") {
      const symbol = String(body.asset || body.base_asset || "").toUpperCase();
      const qty = Number(body.quantity || body.qty || 0);
      const unitPriceUSD = Number(body.price_local || body.unit_price_usd || 0);
