  ...
  for (let i = 0; i < classified.length; i++) {
    ...
    try {
      if (options.dryRun) {
        ...
        logger.debug({ index: i, type: ct.type }, 'Dry run: would process transaction')
        continue
      }
      ...
      if (options.directRecord) {
        ...
        switch (ct.type) {
          case 'EXPENSE':
            if (bankConfig.statementType === 'CREDIT_CARD') {
              result = await recordCreditExpenseVND(config, {
              ...
              result = await recordExpenseVND(config, {
                vnd_amount: ct.amount_vnd,
                date: ct.date,
                counterparty: ct.counterparty || undefined,
                tag: ct.category || undefined,
            ...
            break
          case 'INCOME':
            result = await recordIncomeVND(config, {
              vnd_amount: ct.amount_vnd,
              date: ct.date,
              counterparty: ct.counterparty || undefined,
              tag: ct.category || undefined,
            ...
            break
          case 'CARD_PAYMENT':
            result = await recordCardPaymentVND(config, {
              vnd_amount: ct.amount_vnd,
              date: ct.date,
              note: ct.note || undefined,
              source_ref: ct.source_ref
        ...
        logger.debug({ id: result.transaction_id, type: ct.type, account: result.account_used }, 'Recorded transaction directly')
      } else {
        // Pending action mode - still simplified (no account needed in action_json)
        const actionJson: ActionRequest = {
          action: ct.type === 'INCOME' ? 'income_vnd' :
                  ct.type === 'CARD_PAYMENT' ? 'card_payment_vnd' :
                  bankConfig.statementType === 'CREDIT_CARD' ? 'credit_spend_vnd' : 'spend_vnd',
          params: {
        ...
        const result = await createPendingAction(config, payload, correlationId)
        processedIds.push(result.id)
        logger.debug({ id: result.id, type: ct.type }, 'Created pending action')
      }
    } catch (error: any) {
      logger.error({ error: error.message, index: i }, 'Failed to process transaction')
      errors.push({
        index: i,
        error: error.message,
...
export function getBankConfig(bankName: string): BankStatementConfig {
  ...
  if (!config) {
    throw new Error(`Unknown bank: ${bankName}. Available: ${Object.keys(configs).join(', ')}`)
  }
