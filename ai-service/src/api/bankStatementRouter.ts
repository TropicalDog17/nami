  ...
  (req: Request, res: Response) => {
    ...
    try {
      ...
      res.json({
        ...
        path: req.file.path,
        message:
          "File uploaded successfully. Use POST /api/bank-statement/process to process it.",
      });
    } catch (error: any) {
...
bankStatementRouter.post("/process", async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!fs.existsSync(filePath)) {
      return res
        .status(404)
        .json({ error: "File not found. Please upload again." });
    }
...
bankStatementRouter.post(
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      logger.error(
        { error: error.message },
        "Bank statement upload and processing failed",
      );
      res.status(500).json({ error: error.message });
    }
  },
...
bankStatementRouter.post("/preview", async (req: Request, res: Response) => {
  ...
  try {
    ...
    const expenses = transactions.filter(
      (t) => t.debit && parseFloat(t.debit.replace(/,/g, "")) > 0,
    );
    const income = transactions.filter(
      (t) => t.credit && parseFloat(t.credit.replace(/,/g, "")) > 0,
    );
