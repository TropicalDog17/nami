...
export class HealthChecker {
  ...
  private async checkBackend(
    correlationLogger: any,
  ): Promise<HealthCheckResult["checks"]["backend"]> {
    const startTime = Date.now();
    try {
      // Test backend connectivity by checking pending actions endpoint
      const response = await fetch(
        `${this.cfg.BACKEND_BASE_URL}/admin/pending-actions`,
        {
          method: "GET",
  ...
  private async checkOpenAI(
    correlationLogger: any,
  ): Promise<HealthCheckResult["checks"]["openai"]> {
    const startTime = Date.now();
    try {
  ...
  private checkConfig(
    correlationLogger: any,
  ): HealthCheckResult["checks"]["config"] {
    const issues: string[] = [];

    // Check critical configuration
    if (
      !this.cfg.TELEGRAM_BOT_TOKEN ||
      this.cfg.TELEGRAM_BOT_TOKEN.length < 10
    ) {
      issues.push("Invalid or missing Telegram bot token");
    }

    if (
      !this.cfg.OPENAI_API_KEY ||
      !this.cfg.OPENAI_API_KEY.startsWith("sk-")
    ) {
      issues.push("Invalid or missing OpenAI API key");
    }

    if (
      !this.cfg.BACKEND_BASE_URL ||
      !this.cfg.BACKEND_BASE_URL.startsWith("http")
    ) {
      issues.push("Invalid or missing backend base URL");
    }

    if (
      !this.cfg.BACKEND_SIGNING_SECRET ||
      this.cfg.BACKEND_SIGNING_SECRET.length < 16
    ) {
