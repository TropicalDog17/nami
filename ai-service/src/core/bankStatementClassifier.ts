...
- Output exactly ${transactions.length} items in order`
  ...
  try {
    ...
    return transactions.map((tx, idx) => {
      ...
      const aiData = aiResults.find(r => r.idx === idx) || aiResults[idx] || {}
    ...
    logger.error({ error: error.message }, 'AI batch classification failed, using basic classification')
...
export async function classifyBatch(
  ...
  if (skipAI) {
    ...
    logger.info({ count: results.length }, 'Completed basic classification (no AI)')
    return results
  }
  ...
  const batchPromises = batches.map((batch, idx) =>
    limit(async () => {
      const batchResults = await classifyBankTransactionsBatch(
        llmClient, batch, config, tags, correlationId
      )
 ...
 */
export function getClassificationSummary(transactions: ClassifiedTransaction[]): {
  total: number
  expenses: number
  income: number
  cardPayments: number
  ...
  return {
    ...
    avgConfidence: transactions.length > 0 ? totalConfidence / transactions.length : 0,
    byCategory
  }
}
